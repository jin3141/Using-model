/**
 * Tests for Ising Model Simulation
 * Verifies physical correctness of the implementation
 */
...
function testEnergyCalculationOrdered() {
  ...
  // Each site has 2 bonds (right and down) due to periodic boundary
...
/**
 * Test 4: Energy calculation for antiferromagnetic state
 */
function testEnergyCalculationAntiferro() {
  ...
  // Energy = -J * sum(s_i * s_j) = -J * 2 * size * size * (-1) = +J * 2 * size * size
  const expectedEnergy = J * 2 * size * size;
...
/**
 * Test 5: Low temperature behavior (T -> 0)
 */
function testLowTemperatureBehavior() {
  console.log('\n=== Test 5: Low Temperature Behavior ===');

  const size = 20;
  const temperature = 0.1; // Very low temperature
  const J = 1.0;
  const steps = 1000;
  ...
  // At low temperature, system should stay in ordered state
  // Magnetization should remain high
...
/**
 * Test 6: High temperature behavior (T >> Tc)
 */
function testHighTemperatureBehavior() {
  console.log('\n=== Test 6: High Temperature Behavior ===');

  const size = 20;
  const temperature = 5.0; // High temperature (>> Tc ≈ 2.27)
  const J = 1.0;
  const steps = 1000;
  ...
  // At high temperature, magnetization should be close to 0
  if (magPerSpin > 0.3) {
...
/**
 * Test 7: Metropolis algorithm acceptance
 */
function testMetropolisAcceptance() {
  ...
  // Run a few Monte Carlo steps
  let grid = smallGrid;
  for (let i = 0; i < 10; i++) {
    grid = monteCarloStep(grid, temperature, J);
  }

  // Just verify it runs without errors and produces valid spins
